...
| Response status + payload shape | Success: 302 redirects, no JSON body anywhere. Wrong credentials: Keycloak re-renders the **same** login form at **HTTP 200** — not an error status — so "did the session cookie show up" is the only reliable success signal, not the status code of any one hop. Read it as *did the credential POST's own chain issue* `.SATS_GROUP_AUTH`, not *is the name in the jar*: the hops leading to the form can leave the same name behind (a sign-out clears the cookie by re-setting it empty with `Max-Age=0`, which a jar that only tracks names cannot tell from a real session), and reading the jar alone accepts any password at all. |
...
`{queryName: "clubIds", options: […]}`. A walk that stops at the first match
therefore returns one group and silently drops every other club — which is
exactly what happened when `listClubOptions` was first written: the chooser
showed a partial list (Circus was missing), and because the same function
resolves a stored centre name to its club id, any subscription at a club in a
later group would have stopped resolving and quietly booked nothing. So the
walk collects **every** `clubIds` node and dedupes by id — clubs do repeat
across groups. If the centre list ever looks short again, this is the first
thing to check, and the count is the tell: it should be in the low hundreds,
not a couple of dozen.

**There is no club location anywhere in the page.** A version of the chooser
narrowed the list by country, then city, then club — which is how 226 clubs
ought to be navigated — reading both from the titles the `clubIds` nodes sit
under, since no field carries them. Against the live page every club came back
under one country and one city: the walk found no location titles to read, and
the shipped result was a three-step form whose first two steps each offered a
single meaningless option. It was reverted; the chooser is one flat list that
remembers the last centre instead.

So, for whoever tries again: `label` is a bare club name ("Circus"), not
"Helsinki Circus", and the nesting above a `clubIds` node carries the filter's
own headings rather than places. Grouping needs a source of geography this
page does not have — the club pages elsewhere on the site, or a hard-coded
mapping — and neither is worth it without a fresh capture proving what is
actually there. Verified in production on 2026-08-22, against clubs a real
account can see; not from a capture, because the site is unreachable from the
sandbox this was built in, which is how it shipped blind in the first place.
...
short-circuit on the first *node*, which looks right, type-checks, and passes
every test written against a single-group fixture.

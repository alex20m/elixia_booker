...
describe('buildCalendarFeed', () => {
  ...
  it('still drops a class that was unbooked before it ran, however long ago that was', () => {
    // Attending is what earns a past class its place. A booking given up
    // beforehand was never attended, so it goes the same way a cancelled
    // upcoming one does — the past is not an amnesty.
